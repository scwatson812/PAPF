...
###Specify the file path to folder containing the results from Calculate Test Statistic.R and Simulat the Null Distribution of the Test Statistic.R
...
ots.mat = read.table(paste0(path,'ObservedProportions.txt'),fill = TRUE)
...
for(m in 1:mc.reps){
  for(r in 1:r.rad.vec){
    test.stat.array[m,r] = mean(abs(sts.mat[m,((r-1)*n.points +1):(r*n.points)] - mean.spr.mat[r] ))
  }
}
...
for(r in 1:r.rad.vec){
  observed.test.stat.array[1,r] = mean(abs(ots.mat[r,] - mean.spr.mat[r]))
}
