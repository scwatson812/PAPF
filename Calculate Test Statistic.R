...
########################################
##### Data Generation: One Cluster #####
########################################
# n.selected.points = 40
# p.vec = rep(1,dim(fsa.sh)[1])
# ind =   coordinates(fsa.sh)[,1] > 8264189 & coordinates(fsa.sh)[,2] > 19985702
# p.vec[ind] = 5
# ind <- sample(x = seq(1,dim(fsa.sh)[1]),n.selected.points,replace = FALSE,prob = p.vec/sum(p.vec))
# fsa.sh$selected<- 0
# fsa.sh$selected[ind] = 1

##############################################
##### Data Generation: Multiple Clusters #####
##############################################
# n.selected.points = 40
# n.seed = n.selected.points/10
# selected.points =   sample(n.seed,x = seq(1,dim(fsa.sh)[1]),replace = FALSE)
# p.vec = rep(1,dim(fsa.sh)[1])
# p.vec[selected.points] = 0
# r.factor = 5
# for(i in 1:n.seed){
#   p.vec[W.mat[selected.points[i],]==1] = r.factor
#   p.vec[selected.points] = 0
# }
# selected.points = c(selected.points,sample(n.selected.points - n.seed,x = seq(1,dim(fsa.sh)[1]),replace = FALSE,prob = p.vec/sum(p.vec)))
# fsa.sh$selected<- 0
# fsa.sh$selected[selected.points] = 1

#######################################
##### Data Generation: Dispersion #####
#######################################
# n.selected.points = 40
# selected.points = c()
# p.vec = rep(1,dim(fsa.sh)[1])
# r.factor = 1/10
# for(i in 1:n.selected.points){
#   selected.points = c(selected.points,sample(1,x = seq(1,dim(fsa.sh)[1]),prob = p.vec,replace = FALSE))
#   for(j in 1:length(selected.points)){
#     p.vec[W.mat[selected.points[j],]==1] = r.factor
#   }
#   p.vec[selected.points]=0
# }
# fsa.sh$pvec <- p.vec
...
write.table(observed.area.mat-sum(area(fsa.selected))/sum(area(fsa.sh)),"ObservedPropDiff.txt",row.names = FALSE, col.names = FALSE)
